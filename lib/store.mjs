...
export function createStore(dbFilePath) {
  ...
  /** Админ по умолчанию, если в базе ещё никого с role=admin (раньше сид срабатывал только при полностью пустой таблице — после любой регистрации подсказка на экране переставала работать). */
  function seedAdminIfNeeded(database) {
    const hasAdmin = database.prepare(`SELECT 1 AS x FROM users WHERE role = 'admin' LIMIT 1`).get();
    if (hasAdmin) return;
    const email = normalizeEmail(process.env.ADMIN_EMAIL || "admin@kapsula.local");
    ...
    console.warn(
      `[КАПСУЛА] Создан администратор (в базе не было role=admin): ${email} / пароль из ADMIN_PASSWORD или «admin123». Задайте переменные окружения в продакшене.`
    );
  }
