/** Типы размерных сеток и размеры по категориям (женское / мужское). */
...
export function resolveChartId(product) {
...
export function defaultSizesFor(product) {
  const chartId = resolveChartId(product);
  const chart = SIZE_CHARTS[chartId];
  if (!chart) return ["M"];
  if (chartId === "one_size") return ["One size"];
